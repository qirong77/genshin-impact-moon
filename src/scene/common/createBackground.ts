...
export function createBackground(
    ...
    folder
        .add(config, "positionX", -5, 5)
        .name("X轴位置")
        .onChange(() => {
            plane.position.x = config.positionX;
        });
    folder
        .add(config, "positionY", -5, 5)
        .name("Y轴位置")
        .onChange(() => {
            plane.position.y = config.positionY;
        });
    folder
        .add(config, "positionZ", -10, 10)
        .name("Z轴位置")
        .onChange(() => {
    ...
    folder
        .add(config, "rotationX", -Math.PI, Math.PI)
        .name("X轴旋转")
        .onChange(() => {
            plane.rotation.x = config.rotationX;
        });
    folder
        .add(config, "rotationY", -Math.PI, Math.PI)
        .name("Y轴旋转")
        .onChange(() => {
            plane.rotation.y = config.rotationY;
        });
    folder
        .add(config, "rotationZ", -Math.PI, Math.PI)
        .name("Z轴旋转")
        .onChange(() => {
